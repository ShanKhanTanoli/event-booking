            ...
            @if (Business::StripeConnectAccountID(Auth::user()->id))
                @if (!is_null($account = Stripe::RetrieveAccount(Auth::user()->account_id)))
                    @if (!$account->payouts_enabled)
                        <button wire:click="Complete()" wire:loading.attr='disabled' class="btn btn-primary"
                            type="button">
                            <span wire:loading wire:target="Complete" class="spinner-border spinner-border-sm"
